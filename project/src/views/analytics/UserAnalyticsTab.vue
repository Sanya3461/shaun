<template>
  <div class="user-analytics-tab">
    ...
    <div class="period-title">
      <h2>Last 4 Weeks</h2>
      <p>{{ userProfile?.name || 'User' }}'s activity overview</p>
    </div>
    ...
    <div class="stats-section">
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-number blue">{{ userStats.sessions }}</div>
          <div class="stat-label">Sessions</div>
        </div>
        <div class="stat-card">
          <div class="stat-number blue">{{ userStats.distance }}</div>
          <div class="stat-label">Nautical miles</div>
        </div>
        <div class="stat-card">
          <div class="stat-number blue">{{ userStats.avgSpeed }}</div>
          <div class="stat-unit">kn</div>
          <div class="stat-label">Avg speed</div>
        </div>
        <div class="stat-card">
          <div class="stat-number blue">{{ userStats.maxSpeed }}</div>
          <div class="stat-unit">kn</div>
    ...
    <div class="achievements-section">
      ...
      <div class="achievement-progress-bar merit-out-progress">
        <div class="progress-background">
          <div class="progress-fill merit-out-fill" style="width: 85%;"></div>
      ...
      <div class="achievement-progress-bar distance-champion-progress">
        <div class="progress-background">
          <div class="progress-fill distance-champion-fill" style="width: 60%;"></div>
          <div class="progress-content">
            <div class="achievement-icon">
              <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="event-item">
          <div class="event-logo balaton-logo">
            <img src="https://images.pexels.com/photos/1001682/pexels-photo-1001682.jpeg?auto=compress&cs=tinysrgb&w=48&h=48&fit=crop" 
                 alt="San Francisco Regatta" class="event-logo-img">
          </div>
          <div class="event-details">
            <div class="event-name">San Francisco Regatta</div>
            <div class="event-date">July 20, 14:30</div>
          </div>
        ...
        <div class="event-item">
          <div class="event-logo festival-logo">
            <img src="https://images.pexels.com/photos/416978/pexels-photo-416978.jpeg?auto=compress&cs=tinysrgb&w=48&h=48&fit=crop" 
                 alt="Bay Area Championship" class="event-logo-img">
          </div>
          <div class="event-details">
            <div class="event-name">Bay Area Championship</div>
            <div class="event-date">August 15, 09:00</div>
          </div>
          <ChevronRightIcon class="chevron" />
        </div>
      </div>
    ...
    <div class="crew-section">
      <h3>Crew Members</h3>
      <div v-for="member in userCrewMembers" :key="member.id" class="crew-item">
        <img :src="member.avatar" 
             :alt="member.name" class="crew-avatar">
...
const userCrewMembers = computed(() => userProfileStore.userCrewMembers)

const weekDays = ['M', 'T', 'W', 'T', 'F', 'S', 'S']

// Generate different activity patterns based on user
const calendarDays = computed(() => {
  const basePattern = [
    ...
    { date: 1, activity: 2, currentMonth: true }, 
    { date: 2, activity: 1, currentMonth: true }, 
    { date: 3, activity: 3, currentMonth: true }, 
    { date: 4, activity: 0, currentMonth: true },
    
    // Week 2
    { date: 5, activity: 2, currentMonth: true }, 
    { date: 6, activity: 0, currentMonth: true }, 
    { date: 7, activity: 1, currentMonth: true }, 
    { date: 8, activity: 2, currentMonth: true }, 
    { date: 9, activity: 1, currentMonth: true }, 
    { date: 10, activity: 3, currentMonth: true }, 
    { date: 11, activity: 0, currentMonth: true },
    
    // Week 3
    { date: 12, activity: 1, currentMonth: true }, 
    { date: 13, activity: 2, currentMonth: true }, 
    { date: 14, activity: 0, currentMonth: true }, 
    { date: 15, activity: 3, currentMonth: true }, 
    { date: 16, activity: 1, currentMonth: true }, 
    { date: 17, activity: 2, currentMonth: true }, 
    { date: 18, activity: 0, currentMonth: true },
    
    // Week 4
    { date: 19, activity: 2, currentMonth: true }, 
    { date: 20, activity: 1, currentMonth: true }, 
    { date: 21, activity: 0, currentMonth: true }, 
    { date: 22, activity: 1, currentMonth: true }, 
    { date: 23, activity: 3, currentMonth: true }, 
    { date: 24, activity: 2, currentMonth: true }, 
    { date: 25, activity: 1, currentMonth: true }
  ]
  
  // Modify activity based on user ID for variety
  if (props.userId === 'eszter-kosa') {
    return basePattern.map(day => ({
      ...day,
      activity: Math.min(day.activity + 1, 3) // More active
    }))
  }
...
const userStats = computed(() => {
  ...
  } else if (props.userId === 'imi-vasvary') {
    return {
      sessions: 56,
      distance: 189,
      avgSpeed: 421,
